...
export default function Feed(){
  const [formData, setFormData] = useState({id: 0, email: "", comment: "", date : "", name: "", age: 0});
  const [comments, setComments] = useState<{id: number, email: string, comment: string, date: string, name: string, age: number}[]>([]);
      ...
      <div className=" flex flex-col gap-8 items-center justify-center w-full">
        ...
        <form className="flex flex-col gap-4 w-full max-w-3xl" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col">
            ...
            <input type="text" id="age" className="bg-zinc-900  p-4" value={formData.age} onChange={handleChange} />
          </div>
          <div className="flex flex-col">
      ...
      <div className="p-8">
        ...
        {comments.map(({email, comment, date, name, age}) => (
          <div key={email} className="flex flex-col gap-2 items-start mb-10">
            <p><strong>{name} - {age} anos</strong></p>
