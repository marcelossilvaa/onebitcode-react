  ...
  return (
    <main className="flex justify-center flex-col items-center gap-8 text-white w-full mt-20">
      ...
      <div className="flex flex-col items-center">
        <div
          className={` buttonStyles flex items-center gap-2 !py-2 rounded-xl px-4  ${
            showInput ? `mb-4` : ``
          }`}
        >
          <input
      ...
      <div className="flex gap-8 ">
        <div>
          <button onClick={generate} className="buttonStyles">
            Gerar senha de {showInput ? passwordSize : 8} caracteres!{' '}
          </button>
        </div>

        <button className="buttonStyles" onClick={copyToClipBoard}>
          {copyText}
        </button>
      </div>
      <div>{password}</div>
